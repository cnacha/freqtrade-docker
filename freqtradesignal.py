...
class FreqtradeSignal(LoggingMixin):
    """
    ...
    def send_signal(self):
        ...
        for pair in whitelist:
            ...
            try:
                ...
                if buy and not sell:
                    #send signal to bot
                    pairset = pair.replace("/"," ").split()
                    pairtext =  pairset[1] +"_" +pairset[0]
                    for botconfig in  self.config['bots_to_signal']:
                        signal = {"message_type": "bot",
                              "bot_id": botconfig["bot_id"],
                              "email_token":  botconfig["email_token"],
                              "delay_seconds": 0,
                              "pair":pairtext
                            }

                        response = requests.post('https://3commas.io/trade_signal/trading_view', json=signal)
                        #logger.info(f"sending signal: {signal}")
                    # msg = {
                    #     'type': RPCMessageType.WARNING,
                    #     'status':  f"sending buy signal for {pair}"
                    # }
                    #self.rpc.send_msg(msg)

            except requests.exceptions.RequestException as e:
                logger.warning('Unable to send signal %s: %s', pair, exception)
...
#
    ...
    def check_handle_timedout(self) -> None:
        ...
        :return: None
        """
        #logger.info('check_handle_timedout....')
...
#
    ...
    def update_trade_state(self, trade: Trade, order_id: str, action_order: Dict[str, Any] = None,
        ...
        try:
            ...
            #logger.info(f"... new amount: {new_amount} safevalue:{safe_value_fallback(order, 'filled', 'amount')}")
        ...
        #update trade when the safty order has been closed
        if (order['status'] == 'closed') & ('safety' in order):
            logger.info(f"SO - processing safety order {order} fee: {order['fee']}")
            buy_price = order['price']
            buy_stake =order['cost']
            buy_amount = order['amount']
            buy_fee = order['fee']['cost']

            new_open_rate = ((trade.open_rate * trade.amount) + (buy_price*buy_amount))/ (trade.amount+buy_amount)
            logger.info(f"SO - update rate: {new_open_rate}")
            trade.open_rate = float(f"{new_open_rate:.8f}")
            #trade.fee_open = trade.fee_open + buy_fee
            trade.stake_amount = (buy_stake+ trade.stake_amount)
            trade.recalc_open_trade_value()
            logger.info(f"SO - update trade: {trade}")
            #trade.amount = trade.amount+buy_amount
            # Updating wallets
            self.wallets.update()
